...
contract PredictionMarket {
    ...
    // The following mapping stores the base one index of each participant
    // in the above array. This is needed for removing participants. To remove
    // a participant the value of their specific key is set to zero, hence the base is one.
    mapping (address => uint) baseOneIndex;
    ...
    function addForecast(uint _value) public payable onlyBefore(deadline) {
        ...
        if (forecasts[msg.sender].length == 0) {
            ...
            baseOneIndex[msg.sender] = participants.length;
        }
    ...
    function removeForecast(uint _index) public onlyBefore(deadline) {
        ...
        if (forecasts[msg.sender].length == 0) {
            ...
            participants[baseOneIndex[msg.sender]-1] = participants[tempLength-1];
